...
/*
*  Significant changes by Adrian Del Grosso since the original version of this code:
* 
*  Removed delete option.
*  Added "Save" dialogue type.
* 
* Todo: Refactor this code to use proper strings and a class instead of being
* a gross static thing with hardcoded lengths all over the place.
* 
* This comment block added to ensure compliance with Apache 2.0 license.
* 
*/
...
namespace FileDialog
{
	...
	void ShowFileDialog(bool *open, char *buffer, [[maybe_unused]] unsigned int buffer_size, FileDialogType type = FileDialogType::OpenFile)
	{
		...
		if (open)
		{
			...
			ImGui::SetNextWindowSize(ImVec2(780.0f, 460.0f));
			...
			ImGui::SameLine();
			ImGui::SetCursorPosX(ImGui::GetWindowWidth() - 290);
			...
			if (ImGui::BeginListBox("##TC Version", { 100, 50 }))
			{
				for (int n = 0; n < IM_ARRAYSIZE(versions); n++)
				{
					const bool is_selected = (versions_current_idx == n);
					if (ImGui::Selectable(versions[n], is_selected))
						versions_current_idx = n;
